...
const [activeColor] = useState<{name:string; color:string;}>(colors&& colors.length > 0 ? colors[0] : {name: '', color: ''});
...
const stock = selectedVariant?.stock || 0;
    ...
    return <div className="flex flex-col gap-6 relative">
        ...
        <button className="bg-white border-slate-200 absolute w-full bottom-0 py-3 rounded.3xl flex items-center gap-1 justify-center font-medium hover:bg-stone-100 translate-y-[100%] transition-all duration-300 group-hover:translate-y-0">
            <FiPlus />
            Añadir
        </button>
        </Link>
